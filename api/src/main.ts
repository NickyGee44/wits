...
import { createBurnSignature, createSignature } from './viem';
import { Handler, HandlerEvent } from '@netlify/functions';

const RAW_ADDRESSES = {
  '0x1eF4591f11Db111FaF30EaA21B49d38B1B92a4cf': 10,
...
const handler: Handler = async (event: HandlerEvent) => {
  const { address, type = 'presale' } = event.queryStringParameters;
  if (type === 'burn') {
    ...
    try {
      ...
      const { signature, request } = await createBurnSignature(
        formatted,
        value,
        true
      );
